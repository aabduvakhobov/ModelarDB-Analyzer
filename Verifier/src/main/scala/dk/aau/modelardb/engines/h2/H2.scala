...
class H2(configuration: Configuration, h2storage: H2Storage) extends QueryEngine {
  ...
  def start(): Unit = {
    ...
    //Creates DataPointView and SegmentView Tables
...
//    verifier(configuration, h2storage)
    //Interface
//    QueryInterface.start(configuration, this)
  ...
  def verifier(configuration: Configuration, storage: H2Storage) = {
    val segmentGroups = storage.getSegmentGroups("".asInstanceOf[TableFilter])
  ...
  private def startIngestion(dimensions: Dimensions): Unit = {
    ...
    val timeSeries = Partitioner.initializeTimeSeries(configuration, h2storage.getMaxTid) // Initializes the data source format, source and schema
    val timeSeriesGroups = Partitioner.groupTimeSeries(configuration, timeSeries, h2storage.getMaxGid) //creates time series groups
    h2storage.storeMetadataAndInitializeCaches(configuration, timeSeriesGroups) // initializes model types and
    ...
    this.workingSets = Partitioner.partitionTimeSeries(configuration, timeSeriesGroups, mtidCache, ingestors) //creating parallel workers
